//creating variables (variables are used to store values)
var girlImg, girl
var bgImg, bg
var athenaImg, athena, athenaGroup
var coinImg, coin, coinGroup
var score = 0
var artemis, artemisImg, artemisGroup
var daggerImg, dagger, daggerGroup
var reset, resetImg
var heart1, heart2, heart3, livesImg, lifeCount = 4
var game_over, game_overImg
var playState = 1
var endState = 0
var gameState = playState
var game_over_sound
var dagger_sound
var background_music
var mute_button,mute_buttonImg, audio_buttonImg, audio_button

//this function is used to preload images (and sometimes sound)
function preload(){
girlImg = loadImage ("Images/girl_running.png")
bgImg = loadImage ("Images/greek_mythology_background.jpg")
athenaImg = loadImage ("Images/athena.png")
artemisImg = loadImage ("Images/artemis.png")
daggerImg = loadImage ("Images/dagger.png")
resetImg = loadImage ("Images/reset.png")
game_overImg = loadImage ("Images/game_over.png")
livesImg = loadImage ("Images/heart.png")
coinImg = loadImage ("Images/drachma.png")
game_over_sound = loadSound ("Images/game_over_sound.wav")
dagger_sound = loadSound ("Images/dagger_sound.wav")
coin_sound = loadSound ("Images/coin_sound.wav")
background_music = loadSound ("Images/background_music.wav")
audio_buttonImg = loadImage ("Images/audio_button.png")
mute_buttonImg = loadImage ("Images/mute-button.png")
}

function setup() {

  //this is used to create the canvas/game area
  createCanvas(1000,800)

  //setting the background music and adjusting volume
  background_music.play ()
  background_music.setVolume (0.05)

  //creating background sprite and giving the velocity for movement
  bg = createSprite (500,400,50,50)
  bg.addImage (bgImg)
  bg.scale = 1
  bg.velocityX = -2

  //creating sprite for the main character
  girl = createSprite (200,200,100,100)
  girl.addImage (girlImg)
  girl.scale = 0.4

  //creating groups for all NPC
  athenaGroup = new Group ()

  artemisGroup = new Group ()

  coinGroup = new Group ()

  daggerGroup = new Group ()

  //creating sprites for each heart
  heart1 = createSprite (720, 75, 50, 50)
  heart1.addImage ("heart1", livesImg)
  heart1.scale = 0.1

  heart2 = createSprite (760,75,50,50)
  heart2.addImage ("heart2", livesImg)
  heart2.scale = 0.1

  heart3 = createSprite (800,75,50,50)
  heart3.addImage ("heart3", livesImg)
  heart3.scale = 0.1

  //setting sprite for reset button
  reset = createSprite (500,650,50,50)
  reset.addImage (resetImg)
  reset.scale = 0.5

  //setting sprite for game over animation
  game_over = createSprite (500,475,50,50)
  game_over.addImage (game_overImg)
  game_over.scale = 0.9

  //setting sprite for mute button
  mute_button = createSprite (900,75,60,60)
  mute_button.scale = 0.1
  mute_button.addImage("mute_button",mute_buttonImg)
  mute_button.addImage ("audio_button", audio_buttonImg)
}

function draw() {
  //set background color
  background("white");

  //setting condition for muting the sound (Mouse pressed over for clicking the button)
  if (mousePressedOver(mute_button)) {
    mute ()
  }

  if (gameState === playState) {

    //setting the visiblility to true, so girl is seen
    girl.visible = true
    
    //setting the dagger sound and movement when pressing space key
    if (keyDown ("SPACE")) {
      Dagger ()
      dagger_sound.play ()
    }
  
    //setting condition for dagger touching the Athena group
    if (daggerGroup.isTouching (athenaGroup)) {
      athenaGroup.destroyEach ()
      daggerGroup.destroyEach ()
      score += 15
    }
  
    //setting condition for dagger touching Artmeis group
    if (daggerGroup.isTouching (artemisGroup)) {
      artemisGroup.destroyEach ()
      daggerGroup.destroyEach ()
      score += 15
    }
  
    //setting condition for getting coins
    if (coinGroup.isTouching (girl)) {
      coinGroup.destroyEach ()
      coinGroup.setVelocityXEach(0)
      score += 10
      coin_sound.play ()
    }

    //setting life count if girl touches Athena
    if (girl.isTouching (athenaGroup)) {
      lifeCount = lifeCount - 1
      console.log (lifeCount)
      athenaGroup.destroyEach ()
      score = score - 10
  
      if (lifeCount === 3) {
        heart3.visible = false
      }
  
      if (lifeCount === 2) {
        heart2.visible = false
      }
  
      if (lifeCount === 1) {
        heart1.visible = false
       // gameState = endState
      }
    }
        
    //setting life count if girl touches Athena
    if (girl.isTouching (artemisGroup)) {
    
      lifeCount = lifeCount - 1
      //console.log (lifeCount)
      artemisGroup.destroyEach ()
  
      score = score - 10
      
      if (lifeCount === 3) {
        heart3.visible = false
      }
  
      if (lifeCount === 2) {
        heart2.visible = false
      }
  
      if (lifeCount === 1) {
        heart1.visible = false
        gameState = endState
        game_over_sound.setVolume (0.25)
        game_over_sound.play ()
        //game_over_sound.loop = false
      }
    }
    console.log (gameState)

    //setting background for movement (infinite background)
    if (bg.x < 500) {
      bg.x = 600
    }
 
    //setting girl movement
    if (keyDown (UP_ARROW)){
      girl.y = girl.y - 5
    }

    if (keyDown (DOWN_ARROW)){
      girl.y = girl.y + 5
    }

    //making sure game over and reset button are hidden while playing
    game_over.visible = false
    reset.visible = false
  }
  
  //calling the functions for showing these groups
  Athena ()
  Artemis ()
  Coin ()

  drawSprites ()

  //text for score
  textSize (30)
  fill ("black")
  text ("Score: " + score,100,100)

  if (gameState === endState) {

    //destroying each group when game is over
    athenaGroup.destroyEach ()
    artemisGroup.destroyEach ()
    daggerGroup.destroyEach ()

    //code for hiding the girl
    girl.visible = false

    //stopping the background music
    background_music.stop ()

    //stopping the infinite background
    bg.velocityX = 0

    //stopping the coins' velocity and destroying the,
    coinGroup.setVelocityXEach(0)
    coinGroup.destroyEach ()
    
    //showing the game over and reset button
    game_over.visible = true
    reset.visible = true

    //funtion for if clicking reset button, reset 
    if (mousePressedOver (reset)) {
      //console.log ("reset")
      Reset ()
    }
  }
}

//function for the athena sprite 
function Athena () {
  
  if (frameCount % 150 == 0) {
    athena = createSprite (800,Math.round (random (100,700)),75,75)
    athena.addImage (athenaImg)
    athena.scale = 0.5
    athena.velocityX = -5
    athenaGroup.add (athena)
    athena.setCollider ("rectangle", 30, 0, 70, 300)
    //athena.debug = true
  }
}

//function for the artemis sprite
function Artemis () {
  
  if (frameCount % 230 == 0) {
    artemis = createSprite (800,Math.round (random (100,700)),75,75)
    artemis.addImage (artemisImg)
    artemis.scale = 0.5
    artemis.velocityX = -6
    artemisGroup.add (artemis)
    artemis.setCollider ("rectangle", 30, 0, 70, 300)
    //artemis.debug = true

  }
}

//function for the dagger sprite
function Dagger () {

  dagger = createSprite (girl.x + 50,girl.y,50,50)
  dagger.addImage (daggerImg)
  dagger.scale = 0.3
  dagger.velocityX = 4
  daggerGroup.add (dagger)
}

//function for the reset button
function Reset () {
  game_over.visible = false
  reset.visible = false
  girl.visible = true
  score = 0
  gameState = playState
  bg.velocityX = -2
  heart1.visible = true
  heart2.visible = true
  heart3.visible = true
  coin.visible = true
  lifeCount = 4
}

//function for coin sprite
function Coin(){
  if (frameCount % 175 === 0) {
    coin = createSprite (900, Math.round(random(100,700)), 100, 100)
    coinGroup.add (coin)
    coin.addImage (coinImg)
    coin.scale = 0.2
    coin.velocityX = -5
    coin.setCollider ("rectangle", 30, 0, 70, 300)
    //coin.debug = true

  }
}

//function for switching from audio to mute
function mute () {
  if (background_music.isPlaying ()) {
    background_music.stop ()
    mute_button.changeImage ("mute_button")
  }
  else {
    background_music.play ()
    mute_button.changeImage ("audio_button")
    //audio_button.visible = true
    //mute_button.visible = false
  }
}




